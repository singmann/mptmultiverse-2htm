...
gof_a$model <- ifelse(gof_a$model=="dubep_orig.eqn"|params$model=="jaeger_orig.eqn"|params$model=="koen_orig.eqn"|params$model=="koen_orig.eqn"|params$model=="koen_orig.eqn"|params$model=="heathcote_orig.eqn"|params$model=="heathcote_orig.eqn"|params$model=="dubew_orig.eqn"|params$model=="heathcote2_orig.eqn"|params$model=="koen_orig.eqn",
                       'Q & R Restr.',
                       ifelse(gof_a$model=="dubep_qrest.eqn"|params$model=="jaeger_qrest.eqn"|params$model=="koen_qrest.eqn"|params$model=="heathcote_qrest.eqn"|params$model=="dubew_qrest.eqn"|params$model=="heathcote2_qrest.eqn"|params$model=="koen_qrest.eqn",
                              'Q Restricted',
                              ifelse(gof_a$model=="dubep_rrest.eqn"|params$model=="jaeger_rrest.eqn"|params$model=="koen_rrest.eqn"|params$model=="heathcote_rrest.eqn"|params$model=="dubew_rrest.eqn"|params$model=="heathcote2_rrest.eqn"|params$model=="koen_rrest.eqn",
                                     'R Restricted',
                                     params$model)))
...
ggplot(Core_all, aes(y = est, x = dataset,
  ...
  scale_y_continuous(breaks=seq(0,1,by=.5),limits=c(0,1), 
                     labels = c("0","0.5", "1")) + 
  labs(x='Dataset', y='Estimate', color='Analysis approach', shape='Analysis approach', title='Estimates of Core Parameters Across Data sets for R Restricted')+
  theme_bw() + 
  theme(text=element_text(size = 20))+
  theme(plot.title=element_text(face = 'bold',size=24, hjust = 0.5))+
  coord_flip()

###theme(axis.text.y = element_text(angle = -10, vjust = 0, hjust = 1)) +
...
ggplot(Q_all, aes(y = est, x = dataset,
  ...
  scale_y_continuous(breaks=seq(0,1,by=.5),limits=c(0,1), 
                     labels = c("0","0.5", "1")) + 
  labs(x='Dataset', y='Estimate', color='Analysis approach', shape='Analysis approach', title='Estimates of Q Parameters Across Data sets for R Restricted')+
  theme_bw() + 
  theme(text=element_text(size = 20))+
  theme(plot.title=element_text(face = 'bold',size=24, hjust = 0.5))+
  coord_flip()
...
ggplot(R_all, aes(y = est, x = dataset,
  ...
  scale_y_continuous(breaks=seq(0,1,by=.5),limits=c(0,1), 
                     labels = c("0","0.5", "1")) + 
  labs(x='Dataset', y='Estimate', color='Analysis approach', shape='Analysis approach', title='Estimates of R Parameters Across Data sets for R Restricted')+
  theme_bw() + 
  theme(text=element_text(size = 20))+
  theme(plot.title=element_text(face = 'bold',size=24, hjust = 0.5))+
  coord_flip()

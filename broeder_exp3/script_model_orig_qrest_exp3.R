...
EQN_FILE <- "model_orig_n_qrest.eqn"
check.mpt(EQN_FILE)
### Two model versions:
...
colnames(data) <- c(paste0("OLD_", 1:8), paste0("NEW_", 1:8), paste0("ROLD_", 1:8), paste0("RNEW_", 1:8))
data$id <- factor(paste0("id_",1:nrow(data)))
data$condition <- factor(rep(c("strong_naming","weak_naming"), c(24, 22)))
...
COL_CONDITION <- "condition"  # name of the variable encoding group membership
...
AVAILABLE_CORES <- 3  # Set the number of CPU cores that are available on your computer.
